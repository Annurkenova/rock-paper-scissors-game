<script lang="ts">
  ...
  function handleUsernameSaved(event: CustomEvent) {
    username = event.detail.username // Устанавливаем шаг на FirstStep
  }
...
<WebSocket {selectedItem} {username} />
...
{#if isMobile}
  <main class="app">
    ...
    <Header {username} {score}/>
      {#if step === 1}
      <Menu on:playWithComputer={handlePlayComputer} on:savedUsername={handleUsernameSaved} {username}/>
      {:else if step === 2}
        <FirstStep
          on:setpage={handleSetPage}
          bind:selectedItem
          on:playersReady={handlePlayComputer}
        />
      {:else if step === 3}
        <SecondStep
          {selectedItem}
          ...
          on:setpage={handleSetPage}
        />
        <!-- Перемещаем SecondStep на третье место -->
      {:else if step === 4}
        <ThirdStep
          bind:computerItem
          {selectedItem}
        ...
        />
        <!-- Перемещаем ThirdStep на четвертое место -->
      {/if}
    ...
    {#if showRules}
      <div class="rules-page">
        <Rules
          {step}
          on:setpage={handleRulesClosed}
          on:closeRules={handleRulesClosed}
        />
      </div>
    {/if}
  </main>
  ...
  <main class="app-desktop">
    <span class="background {showRules ? 'blurred' : 'not-blurred'}">
      ...
      <div class="desktop-content">
        ...
        <Menu on:playWithComputer={handlePlayComputer} on:savedUsername={handleUsernameSaved} {username}/>
        {:else if step === 2}
          <FirstStep bind:selectedItem on:setpage={handleSetPage} />
        {:else if step === 3}
          <SecondStep
            bind:selectedItem
            bind:computerItem
